...
Menu = {0 : "Exit",1 : "Read data info from dataset",2 : "Show all the result",3 : "Search by exact school name",4 : "Search school that contains the following word",
        5 : "Display all the subject offer",6: "Search school that offer the subject search",7: "textpredictive for displaying all the result that match the word entered"}
...
def textpredictive(user_input):
    autosuggestschool = filter(lambda x: x.startswith(user_input), Subjects_offered.keys())
    if len(autosuggestschool) > 0:
        for i in autosuggestschool:
            print (i)
...
while True:
    ...
    if option == 0:
        ...
        user_input = raw_input("School starting with ? ")
        textpredictive(user_input.upper())
    else:
